...
/**
 * AllIn1Rentals R2 Migration Script - CARS
 * 
...
const CONFIG = {
  ...
  csvUrl: "https://docs.google.com/spreadsheets/d/1sVPsjAyRPgIw7syL0kjcRr9MfusSgHKFayFqjwO-f5M/export?format=csv&gid=569293808",
  category: "cars",
  outputFile: "cars-new.json",
  errorLogFile: "error_logs_cars.txt",
  ...
  tempDir: join(tmpdir(), "allin1rentals-cars"),
};
...
// Column mapping for cars CSV
// CSV columns: Sr. No,Product Name,Tagline,Slugs,Gear,Description,Gear Type,PRICE,Top Speed,SEATS,Title Video Link,Title Image Link,Dropbox Link
...
async function fetchProducts(): Promise<Product[]> {
  ...
  console.log(`Found ${products.length} cars\n`);
  return products;
}
...
async function main() {
  ...
  // Parse command line args for starting index
  const startIndex = parseInt(process.argv[2] || "0", 10);
  
  console.log("\n" + "=".repeat(70));
  console.log("ALLIN1RENTALS CARS MIGRATION");
  ...
  let results: any[] = [];
  if (startIndex > 0 && existsSync(CONFIG.outputFile)) {
    try {
  ...
  let success = results.length, fail = 0;
  ...
  for (let i = startIndex; i < products.length; i++) {
    ...
    try {
      ...
      if (migrationResult) {
        ...
        // Clean slug - remove leading/trailing slashes and normalize
        ...
        console.log(`   SUCCESS (saved ${results.length} products)\n`);
      } else {
        fail++;
        console.log(`   FAILED\n`);
      }
