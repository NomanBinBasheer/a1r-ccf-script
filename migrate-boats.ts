...
const CONFIG = {
  ...
  csvUrl:
    "https://docs.google.com/spreadsheets/d/1-aTjhhQocXZVvdhMhwr1HcaHOmleZu1xEe39r4Y-WSs/export?format=csv&gid=0",
  category: "boats",
...
async function convertVideoToMp4(
  ...
  const inputPath = join(
    CONFIG.tempDir,
    `in_${timestamp}_${sanitizeName(originalFilename)}`,
  );
  ...
  const outputPath = join(
    CONFIG.tempDir,
    `out_${timestamp}_${sanitizeName(outputFilename)}`,
  );
  ...
  try {
    ...
    if (compress) {
      ...
      ffmpegArgs.push(
        "-bufsize",
        `${CONFIG.media.videoTargetBitrateMbps * 2}M`,
      );
    }
...
async function processVideo(
  ...
  if (needsConversion || needsCompression) {
    console.log(
      `      ${needsConversion ? "Converting" : "Compressing"} video...`,
    );
    const result = await convertVideoToMp4(buffer, filename, needsCompression);
    console.log(
      `      ${sizeMB.toFixed(1)}MB -> ${(result.buffer.length / 1024 / 1024).toFixed(1)}MB`,
    );
    return result;
  }
...
async function compressImage(
  ...
  if (sizeMB <= CONFIG.media.compressImagesOverMB || !checkImageMagick())
    return inputBuffer;
  ...
  try {
    ...
    console.log(
      `      Image: ${sizeMB.toFixed(1)}MB -> ${(result.length / 1024 / 1024).toFixed(1)}MB`,
    );
    return result;
  } catch {
    return inputBuffer;
...
function isGoogleDriveLink(url: string): boolean {
  return (
    url.includes("drive.google.com") || url.includes("docs.google.com/file")
  );
}
 ...
 */
async function migrateMediaFolder(
  folderUrl: string,
  productName: string,
  boatId: string,
  ...
  if (isGoogleDriveLink(folderUrl)) {
    ...
    const gdriveResult = await gdriveDownloadFolder(
      folderUrl,
      boatId,
      productName,
    );
    ...
    if (!gdriveResult.success) {
      logError(
        "GDriveDownload",
        new Error(gdriveResult.error || "Unknown error"),
        { folderUrl, productName, boatId },
      );
      return null;
    }
    ...
    const videos = gdriveResult.files
      .filter((f) => getFileType(f.name) === "video")
      .sort((a, b) => a.name.localeCompare(b.name));
    const images = gdriveResult.files
    ...
    for (let i = 0; i < orderedFiles.length; i++) {
      ...
      console.log(
        `   [${i + 1}/${orderedFiles.length}] ${file.name} (${originalSize}MB)`,
      );
    ...
    console.log(
      `   Done: ${videoUrls.length} videos, ${imageUrls.length} images`,
    );
    return { galleryContent, titleImage, titleVideo };
  }
  ...
  if (hasBrokenLink(boatId)) {
    ...
    logError("BrokenLink", new Error("Dropbox returns HTML instead of ZIP"), {
      folderUrl,
      productName,
      boatId,
    });
  ...
  if (streamingInfo.useStreaming) {
    ...
    const streamResult = await streamingDropboxMigration(
      folderUrl,
      boatId,
      productName,
    );
    ...
    if (!streamResult.success) {
      logError(
        "StreamingDownload",
        new Error(streamResult.error || "Unknown error"),
        { folderUrl, productName, boatId },
      );
      return null;
    }
    ...
    const videos = streamResult.files
      .filter((f) => getFileType(f.name) === "video")
      .sort((a, b) => a.name.localeCompare(b.name));
    const images = streamResult.files
      .filter((f) => getFileType(f.name) === "image")
      .sort((a, b) => a.name.localeCompare(b.name));
    const orderedFiles = [...videos, ...images];
...
async function main() {
  ...
  try {
    await r2Client.send(
      new HeadBucketCommand({ Bucket: CONFIG.r2.bucketName }),
    );
    console.log("R2 OK\n");
  } catch (e: any) {
  ...
  // Determine end index
  const endIndex = endIndexArg
    ? Math.min(parseInt(endIndexArg, 10), products.length)
    : products.length;
  ...
  for (let i = startIndex; i < endIndex; i++) {
    ...
    if (existingIds.has(product.id)) {
      console.log(
        `[${i + 1}/${products.length}] ${product.name} - ALREADY DONE, skipping`,
      );
      skipped++;
      continue;
    }
    ...
    try {
      const migrationResult = await migrateMediaFolder(
        product.dropboxLink,
        product.name,
        product.id,
      );
      ...
      if (migrationResult) {
        const isValidMediaUrl = (url: string): boolean => {
          ...
          return (
            /\.(jpg|jpeg|png|gif|webp|mp4|mov|avi|mkv)$/i.test(url) ||
            url.includes("r2.dev")
          );
        };
        ...
        // Clean slug
        let cleanSlug =
          product.slug || sanitizeName(product.name).toLowerCase();
  ...
  console.log(
    `New success: ${success - (results.length - (success - skipped))}`,
  );
  ...
  if (results.length > 0) {
    const totalVideos = results.reduce(
      (sum, p) =>
        sum +
        p.galleryContent.filter((u: string) => getFileType(u) === "video")
          .length,
      0,
    );
    const totalImages = results.reduce(
      (sum, p) =>
        sum +
        p.galleryContent.filter((u: string) => getFileType(u) === "image")
          .length,
      0,
    );
